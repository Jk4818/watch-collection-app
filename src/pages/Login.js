      ...
      <div className="w-full h-full flex items-center justify-center">
        <div className='w-96 h-96 text-center flex flex-col gap-4'>
          ...
          <form onSubmit={registerWithEmailAndPassword} className='flex flex-col gap-4 font-Actor text-center' >
            <input onChange={(e) => setSignInEmail(e.target.value)} className='h-12 border-2 border-black rounded-md p-2 bg-main focus:bg-white focus:outline-none focus:border-blue-500 ' name="email" type="email" placeholder="Email" required/>

            <input onChange={(e) => setSignInPassword(e.target.value)} className='h-12 border-2 border-black rounded-md p-2 bg-main focus:bg-white focus:outline-none focus:border-blue-500 ' name="password" type="password" placeholder="Password" required/>
            <button className='h-12 border-2 border-black rounded-md bg-black text-main font-bold tracking-wide' type="submit">Log in</button>

            <div>Don't have an account? <span className='text-blue-600 hover:underline hover:underline-offset-1 hover:font-semibold'><Link to="/signup">Create Account</Link></span></div>
          </form>
